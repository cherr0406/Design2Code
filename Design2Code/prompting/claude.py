...
import PIL
import retry
from gpt4v_utils import (
    cleanup_response,
    encode_image,
    extract_text_from_html,
    index_text_from_html,
)
        ...
        messages=[
            {
                ...
                "content": [
                    ...
                    {"type": "text", "text": "Current Webpage:"},
                    {
                        "type": "image",
...
def visual_revision_prompting(client, model, input_image_file, original_output_image):
    ...
    html = claude_revision_call(
        client, model, input_image, original_output_image, prompt
    )
...
def layout_marker_prompting(client, model, image_file, auto_insertion=False):
    ...
    take_screenshot(
        image_file.replace(".png", "_marker.html"),
        image_file.replace(".png", "_marker.png"),
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--prompt_method",
        type=str,
        default="text_augmented_prompting",
        help="prompting method to be chosen from {direct_prompting, text_augmented_prompting, revision_prompting, layout_marker_prompting}",
    )
    parser.add_argument(
        "--orig_output_dir",
        type=str,
        default="gpt4v_text_augmented_prompting",
        help="directory of the original output that will be further revised",
    )
    parser.add_argument(
        "--model", type=str, default="claude-3-opus-20240229", help="model to benchmark"
    )
    parser.add_argument(
        "--file_name", type=str, default="all", help="any particular file to be tested"
    )
    parser.add_argument(
        "--subset",
        type=str,
        default="testset_100",
        help="evaluate on the full testset or just a subset (choose from: {testset_100, testset_full})",
    )
    parser.add_argument(
        "--take_screenshot",
        action="store_true",
        help="whether to render and take screenshot of the webpages",
    )
    parser.add_argument(
        "--auto_insertion",
        type=bool,
        default=False,
        help="whether to automatically insert texts into marker positions",
    )
    ...
    if args.prompt_method == "direct_prompting":
        ...
        predictions_dir = (
            cache_dir
            + f"{args.model}_layout_marker_prompting"
            + ("_auto_insertion" if args.auto_insertion else "")
        )
    ...
    if args.file_name == "all":
        test_files = [
            ...
            and "_marker" not in item
            and item not in existing_predictions
        ]
    else:
    ...
    for filename in tqdm(test_files):
        ...
        if os.path.exists(
            os.path.join(predictions_dir, filename.replace(".png", ".html"))
        ) and os.path.exists(os.path.join(predictions_dir, filename)):
        ...
        try:
            if args.prompt_method == "direct_prompting":
                html = direct_prompting(
                    client, args.model, os.path.join(test_data_dir, filename)
                )
            elif args.prompt_method == "text_augmented_prompting":
                html = text_augmented_prompting(
                    client, args.model, os.path.join(test_data_dir, filename)
                )
            elif args.prompt_method == "revision_prompting":
                html = visual_revision_prompting(
                    client,
                    args.model,
                    os.path.join(test_data_dir, filename),
                    os.path.join(orig_data_dir, filename),
                )
            elif args.prompt_method == "layout_marker_prompting":
                html = layout_marker_prompting(
                    client,
                    args.model,
                    os.path.join(test_data_dir, filename),
                    auto_insertion=args.auto_insertion,
                )

            with open(
                os.path.join(predictions_dir, filename.replace(".png", ".html")), "w"
            ) as f:
                f.write(html)
